...
class SoundDetector: ObservableObject {
    ...
    func setFlashlightMode(for title: String) {
        DispatchQueue.main.async {
            ...
            default:
                print("Unknown flashlight mode: \(title)")
            }
        }
    }
    ...
    var delegate: SoundDetectorDelegate?
    ...
    func showPermissionAlert() {
        let alert = UIAlertController(
            ...
            message: "This app needs access to your microphone to detect claps and play sound with flashlight effects. Please enable it in Settings.",
    ...
    private func playAlarm(for duration: TimeInterval, soundName: String) {
        ...
        do {
            ...
            DispatchQueue.main.async {
                self.stopTimer = Timer.scheduledTimer(withTimeInterval: duration, repeats: false) { [weak self] _ in
                    self?.stopAlarm()
                    self?.stopTorchEffect()
                    self?.delegate?.didFinishAlarm()
                }
